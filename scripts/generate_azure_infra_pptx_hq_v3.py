...
OUTPUT = Path("artifacts/Azure_Infra_Deployment_Overview_HQ_v3.pptx")
...
def add_background(slide, subtitle: str = "") -> None:
    ...
    top = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE, Inches(0), Inches(0), Inches(SLIDE_W), Inches(0.42))
    top.fill.solid()
    top.fill.fore_color.rgb = NAVY
    top.line.color.rgb = NAVY

    accent = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE, Inches(0), Inches(0.42), Inches(SLIDE_W), Inches(0.05))
    accent.fill.solid()
    accent.fill.fore_color.rgb = AZURE
    accent.line.color.rgb = AZURE

    badge = slide.shapes.add_textbox(Inches(10.4), Inches(0.08), Inches(2.7), Inches(0.24))
    btf = badge.text_frame
    btf.clear()
    bp = btf.paragraphs[0]
    bp.text = "aviation-demos-01 | Azure"
    bp.font.size = Pt(9)
    bp.font.bold = True
...
def build_presentation(data: dict[str, Any]) -> None:
    ...
    add_speaker_notes(
        s1,
        [
            "This deck summarizes the essential infrastructure migration into the target tenant.",
            "Data stack migration is on a parallel track; this view focuses on runtime readiness.",
            f"All evidence shown was collected on {data['generated_at']}.",
        ],
    )

    # Slide 2: Executive summary
    s2_exec = prs.slides.add_slide(prs.slide_layouts[6])
    add_background(s2_exec, "Executive framing: deployment readiness, risk, and immediate next actions")
    add_title(s2_exec, "Executive Summary", "Current state and deployment decision posture")
    ...
    add_card(
        ...
        "Readiness snapshot",
        [
            "Essential infra is deployed and operational in target tenant account.",
            "Frontend and backend are live; AKS backend reports 2/2 ready replicas.",
            "Core shared dependencies (Azure OpenAI, AI Search, PostgreSQL) are healthy.",
            "CI/CD runs via GitHub OIDC federation with scoped runtime permissions.",
        ],
        border=GREEN,
    )
    ...
    add_card(
        ...
        "Decision",
        [
            "Proceed with deployment operations while completing two hardening tasks:",
            "1) enforce App Service HTTPS-only, 2) cut backend route to internal LB.",
        ],
        border=ORANGE,
    )

    add_pill(s2_exec, 9.4, 1.8, "Go-Live", "YES (WITH TASKS)", GREEN)
    add_pill(s2_exec, 9.4, 3.0, "Critical Risks", "LOW", GREEN)
    add_pill(s2_exec, 9.4, 4.2, "Blocking Items", "NONE", AZURE)
    add_pill(s2_exec, 9.4, 5.4, "Open Hardening", "2", ORANGE)
    add_speaker_notes(
        s2_exec,
        [
            "Bottom line: infrastructure is ready and running in target tenant.",
            "The only remaining work is hardening, not functional enablement.",
            "No blockers identified for continuing deployment operations.",
        ],
    )

    # Slide 3: Scope and migration cut
    ...
    add_speaker_notes(
        s2,
        [
            "This slide confirms exactly what moved under this deployment scope.",
            "Data-stack migration remains a separate execution stream to avoid cross-team coupling.",
            "Networking and CI/CD controls are included because they are essential runtime infrastructure.",
        ],
    )
    ...
    add_speaker_notes(
        s3,
        [
            "User requests terminate at frontend App Service and are forwarded to backend on AKS.",
            "Backend fans out to Azure OpenAI, AI Search, and PostgreSQL as needed.",
            "Internal LB is already provisioned for private-only routing cutover.",
        ],
    )
    ...
    add_speaker_notes(
        s4,
        [
            "Trust zones are separated between public ingress, runtime VNet, and shared services.",
            "PostgreSQL private endpoint and private DNS are in place for private connectivity.",
            "The architecture already supports reducing public exposure after final cutover.",
        ],
    )
    ...
    add_speaker_notes(
        s5,
        [
            "CI/CD uses OIDC federation to authenticate from GitHub Actions to Azure.",
            "This avoids long-lived secrets in the repository or workflow variables.",
            "Deployments target both AKS backend and App Service frontend paths.",
        ],
    )
    ...
    add_speaker_notes(
        s6,
        [
            "Inventory table lists the required components for this deployment cut.",
            "All listed components are present and in healthy provisioning/runtime states.",
        ],
    )
    ...
    add_speaker_notes(
        s7,
        [
            "Operational checks include deployment readiness and live health endpoints.",
            "Backend, frontend, and shared AI/data dependencies are responding as expected.",
            "This is runtime evidence, not just IaC provisioning confirmation.",
        ],
    )
    ...
    add_speaker_notes(
        s8,
        [
            "Current control posture is solid for runtime operation.",
            "Hardening backlog is intentionally sequenced to minimize deployment disruption.",
            "Top priorities are HTTPS-only and private backend routing cutover.",
        ],
    )
    ...
    add_speaker_notes(
        s9,
        [
            "Checklist confirms required infrastructure components are in place.",
            "Go-live is supported with no blockers; hardening tasks remain open but non-blocking.",
            "Recommended next checkpoint is post-hardening validation.",
        ],
    )

...
Loads normalized ASRS records into SQLite (dev) or PostgreSQL (prod).
"""
...
def upsert_sqlite(cursor: sqlite3.Cursor, record: Dict[str, str], ingested_at: str) -> None:
    cursor.execute(
        """
...
def load_sqlite(db_path: str, records_file: Path, run_id: str, source_manifest_path: str, batch_size: int) -> None:
    ...
    for record in iter_jsonl(records_file):
        ...
        try:
            upsert_sqlite(cursor, record, ingested_at)
            loaded += 1
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Load ASRS records into SQLite or PostgreSQL")
    parser.add_argument("--mode", choices=["sqlite", "postgres"], default="sqlite", help="Database mode")
    parser.add_argument("--db", default="aviation.db", help="SQLite database path")
    ...
    if args.mode == "sqlite":
        load_sqlite(args.db, records_file, run_id, source_manifest, args.batch_size)
    else:

...
AZURE_OPENAI_ORCHESTRATOR_DEPLOYMENT_NAME="${AZURE_OPENAI_ORCHESTRATOR_DEPLOYMENT_NAME:-gpt-5-nano}"
...
AZURE_OPENAI_VOICE_MODEL="${AZURE_OPENAI_VOICE_MODEL:-gpt-4o-mini-tts}"
...
SCHEMA_CACHE_TTL_SECONDS="${SCHEMA_CACHE_TTL_SECONDS:-300}"
KQL_SCHEMA_MODE="${KQL_SCHEMA_MODE:-static}"
...
GRAPH_RETRY_BACKOFF_SECONDS="${GRAPH_RETRY_BACKOFF_SECONDS:-0.25}"
GRAPH_CIRCUIT_BREAKER_FAIL_THRESHOLD="${GRAPH_CIRCUIT_BREAKER_FAIL_THRESHOLD:-3}"
GRAPH_CIRCUIT_BREAKER_OPEN_SECONDS="${GRAPH_CIRCUIT_BREAKER_OPEN_SECONDS:-30}"
...
WEBAPP_SETTINGS=(
  ...
  "AZURE_OPENAI_VOICE_MODEL=$AZURE_OPENAI_VOICE_MODEL"
...
if [ -n "$AZURE_OPENAI_MANAGED_IDENTITY_CLIENT_ID" ]; then
  WEBAPP_SETTINGS+=("AZURE_OPENAI_MANAGED_IDENTITY_CLIENT_ID=$AZURE_OPENAI_MANAGED_IDENTITY_CLIENT_ID")
fi
if [ -n "$AZURE_OPENAI_TENANT_ID" ]; then
  WEBAPP_SETTINGS+=("AZURE_OPENAI_TENANT_ID=$AZURE_OPENAI_TENANT_ID")
fi
if [ -n "$AZURE_OPENAI_CLIENT_ID" ]; then
  WEBAPP_SETTINGS+=("AZURE_OPENAI_CLIENT_ID=$AZURE_OPENAI_CLIENT_ID")
fi
if [ -n "$AZURE_OPENAI_CLIENT_SECRET" ]; then
  WEBAPP_SETTINGS+=("AZURE_OPENAI_CLIENT_SECRET=$AZURE_OPENAI_CLIENT_SECRET")
fi
...
if bool_true "$DEPLOY_K8S"; then
  ...
  export AZURE_OPENAI_ORCHESTRATOR_DEPLOYMENT_NAME
  ...
  export FLASK_ENV
  export LOG_LEVEL
  export SCHEMA_CACHE_TTL_SECONDS
  export KQL_SCHEMA_MODE
  ...
  export GRAPH_RETRY_BACKOFF_SECONDS
  export GRAPH_CIRCUIT_BREAKER_FAIL_THRESHOLD
  export GRAPH_CIRCUIT_BREAKER_OPEN_SECONDS
...
echo "  AZURE_OPENAI_VOICE_MODEL=${AZURE_OPENAI_VOICE_MODEL}"

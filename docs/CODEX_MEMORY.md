...
- 2026-02-24: Deployed undeployed `main` commits `190b0d8` and `751fafd` to backend AKS and frontend App Service.
  - Decision: Built and pushed backend image `avrag705508acr.azurecr.io/aviation-rag-backend:backend-751fafd-manual-20260224195804` from committed `HEAD` (`751fafd`) via `docker buildx --platform linux/amd64`, verified ACR manifest metadata (`os=linux`, `architecture=amd64`), rolled `aviation-rag-backend` deployment to the new tag, rebuilt frontend, packaged standalone output to `/tmp/app.zip`, and deployed via OneDeploy (`deploymentId=952e8263-6a40-40d0-a2c3-56dfce4a00fa`, `status=RuntimeSuccessful`).
  - Why: Publish undeployed datastore-alignment and chat-scroll fixes while preserving tenant/subscription and `amd64` deployment guardrails.
  - Sources: `git log --oneline -n 12`, `git show 190b0d8`, `git show 751fafd`, `./scripts/aks/use-deploy-target-context.sh`, `./scripts/validate-tenant-lock.sh`, `git archive --format=tar HEAD | docker buildx build --platform linux/amd64 -f Dockerfile.backend --tag ... --push -`, `az acr repository show-manifests --name avrag705508acr --repository aviation-rag-backend --detail --orderby time_desc -o json`, `kubectl -n aviation-rag set image deployment/aviation-rag-backend ...`, `kubectl -n aviation-rag rollout status deployment/aviation-rag-backend --timeout=900s`, `curl http://127.0.0.1:18081/health`, `curl http://127.0.0.1:18081/api/fabric/preflight`, `bash -lc 'cd src && npm run -s build'`, `az webapp deploy --resource-group rg-aviation-rag --name aviation-rag-frontend-705508 --src-path /tmp/app.zip --type zip --async true --restart true`, `curl https://aviation-rag-frontend-705508.azurewebsites.net/api/health`, `curl -I https://aviation-rag-frontend-705508.azurewebsites.net/chat`, `curl https://aviation-rag-frontend-705508.azurewebsites.net/api/fabric/preflight`.
  - Changed-from: Backend image `avrag705508acr.azurecr.io/aviation-rag-backend:backend-f130301-manual-20260224190036`; previous frontend deployment `deploymentId=d705a574-5368-4a83-97ce-dfb383a276d5`.
...
- 2026-02-24: Deployed latest undeployed commits (`f7512f3`, `498c8bf`) to backend AKS and frontend App Service.
  ...
  - Sources: `git log --oneline -n 5`, `git diff --name-only 7eef081..HEAD`, `./scripts/validate-tenant-lock.sh`, `./scripts/aks/use-deploy-target-context.sh`, `git archive --format=tar HEAD | docker buildx build --platform linux/amd64 -f Dockerfile.backend --tag ... --push -`, `az acr repository show-manifests --name avrag705508acr --repository aviation-rag-backend --detail --orderby time_desc -o json`, `kubectl -n aviation-rag set image deployment/aviation-rag-backend backend=...`, `kubectl -n aviation-rag rollout status deployment/aviation-rag-backend --timeout=900s`, `kubectl -n aviation-rag port-forward svc/aviation-rag-backend-internal 18081:80`, `curl http://127.0.0.1:18081/health`, `curl http://127.0.0.1:18081/api/fabric/preflight`, `cd src && npm run build`, `az webapp deploy --resource-group rg-aviation-rag --name aviation-rag-frontend-705508 --src-path /tmp/app.zip --type zip --async true --restart true`, `curl https://aviation-rag-frontend-705508.azurewebsites.net/api/health`, `curl https://aviation-rag-frontend-705508.azurewebsites.net/api/fabric/preflight`.
  - Changed-from: Backend image `avrag705508acr.azurecr.io/aviation-rag-backend:backend-7eef081-manual-20260224150731`.
- 2026-02-24: Deployed undeployed `main` backend commits through `7eef081` to AKS.
  - Decision: Built and pushed backend image `avrag705508acr.azurecr.io/aviation-rag-backend:backend-7eef081-manual-20260224150731` from committed `HEAD` (`7eef081`) using `docker buildx --platform linux/amd64`, verified ACR manifest metadata (`os=linux`, `architecture=amd64`), rolled `aviation-rag-backend` deployment to the new tag, and validated backend `/health` plus `/api/fabric/preflight` through internal service port-forward.
  - Why: Publish undeployed query-routing and SQL/Fabric fallback fixes (`2a86fe0`, `02d620d`, `3237a25`, `c38c61f`, `7eef081`) while preserving tenant/subscription and platform guardrails.
  - Sources: `git log --oneline --reverse 3177058..HEAD`, `az account show --query "{user:user.name,tenantId:tenantId,id:id,name:name}" -o json`, `./scripts/validate-tenant-lock.sh`, `./scripts/aks/use-deploy-target-context.sh`, `git archive --format=tar HEAD | docker buildx build --platform linux/amd64 -f Dockerfile.backend --tag ... --push -`, `az acr repository show-manifests --name avrag705508acr --repository aviation-rag-backend --detail --orderby time_desc -o json`, `kubectl -n aviation-rag set image deployment/aviation-rag-backend backend=...`, `kubectl -n aviation-rag rollout status deployment/aviation-rag-backend --timeout=900s`, `kubectl -n aviation-rag get deployment aviation-rag-backend -o jsonpath=...`, `kubectl -n aviation-rag port-forward svc/aviation-rag-backend-internal 18081:80`, `curl http://127.0.0.1:18081/health`, `curl http://127.0.0.1:18081/api/fabric/preflight`.
  - Changed-from: Backend image `avrag705508acr.azurecr.io/aviation-rag-backend:backend-02d620d-amd64fix-20260224113214`.
- 2026-02-24: Added dark-launch predictive optimization runtime path (API/UI + deployment flags).
  - Decision: Introduced additive predictive endpoints (`/api/predictive/delays`, `/api/predictive/delay-metrics`, `/api/predictive/actions`, `/api/predictive/decision-metrics`), left-sidebar-bottom `Predictive Ops` entry behind frontend flags, backend/runtime deployment flags for dark launch, and Postgres mirror deployment assets (`scripts/predictive/00_create_predictive_tables.sql`, `scripts/predictive/07_mirror_fabric_predictions_to_postgres.py`, `docs/PREDICTIVE_OPTIMIZATION_DEPLOYMENT.md`).
  - Why: Implement prediction-optimization capability without changing existing chat/retrieval contracts and with immediate feature-flag rollback support.
  - Sources: `src/api_server.py`, `src/predictive_delay_service.py`, `src/components/layout/sidebar.tsx`, `src/components/predictive/predictive-panel.tsx`, `src/app/chat/page.tsx`, `src/app/api/predictive/*`, `k8s/backend-configmap.yaml`, `scripts/render-k8s-manifests.sh`, `.github/workflows/deploy-backend.yaml`, `.github/workflows/deploy-frontend.yaml`.
  - Changed-from: No dedicated predictive API/UI path and no predictive deployment flags/mirror runbook.
